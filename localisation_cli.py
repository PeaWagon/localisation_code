...
def print_intro():
    print(
        ...
        "Please have python version 3 installed on your computer.",
...
def do_analysis(input_name, operating_system):
    ...
    default_localisation = Localisation(
        input_name, operating_system,
        {}, {}, 1, False, {}, {}, {}, 0, 0
    )
    default_localisation.open_input()       # open input file, complete dict1
    default_localisation.ask_organise()     # ask whether to organise files by localisation
    default_localisation.choose_NL_split()  # determine NL_split value using dict1
    default_localisation.make_dict_main()   # initialise dict_main using NL_split
    default_localisation.analyse_dict()     # sort dict1 into dict_main

    # write data into output file
    if default_localisation.write_dict_main() != "onecell":
        default_localisation.plot_data()
    # if only one cell in dataset, cannot organise
    # by localisation
    else:
        return
    ...
    if default_localisation.sorter is True:
        if len(default_localisation.local0) != 0:
            nopole = Localisation(
                g + "_no_local.csv", operating_system,
                default_localisation.local0,
                {}, 1, False, {}, {}, {}, 0, 0
            )
        ...
        if len(default_localisation.local1) != 0:
            onepole = Localisation(
                g + "_one_local.csv",
                operating_system,
                default_localisation.local1,
                {}, 1, False, {}, {}, {}, 0, 0
            )
        ...
        if len(default_localisation.local2) != 0:
            twopole = Localisation(
                g + "_two_local.csv", operating_system,
                default_localisation.local2,
                {}, 1, False, {}, {}, {}, 0, 0
            )
        ...
        if len(default_localisation.local1) * len(default_localisation.local2) != 0:
            p1 = deepcopy(default_localisation.local1)
            p2 = deepcopy(default_localisation.local2)
            for item in p2:
                p1[item] = p2[item][:]
            polar = Localisation(
                g + "_polar.csv",
...
def get_dir_ID(current_OS):
    ...
    if dir_ID == "":
        ...
        print(
            "Identifier cannot be part of \"__pycache__\".",
            "Please try again."
        )
        return "error"
...
def make_group_list(dir_ID, current_OS):
    ...
    if len(make_list) == 0:
        print(
            "No folders found with identifier ",
            str(dir_ID), ". Please try again.",
            sep=""
        )
        return "empty"
    # loop through folders to make sure all are for analysis
    else:
        print("Looking for folders in:", str(start_dir))
        print("\nFolders are:", str([x[:-4] for x in make_list]), "\n")
        make_list2 = make_list[:]   # make sure that looping removes from
        for group in make_list:     # a hard copied list
            print("\nWould you like to analyse data in folder " + str(group[:-4]) + "? The options are:")
...
def prep_for(current_OS, make_list):
    ...
    for i, group in enumerate(make_list):
        ...
        if os.path.isfile(new_dir + "/" + str(group)) is True:
            os.remove(new_dir + "/" + str(group))
        if os.path.isfile(new_dir + "/" + g + "_output.csv") is True:
            os.remove(new_dir + "/" + g + "_output.csv")
        if os.path.isfile(new_dir + "/" + g + "_localisation_data.csv") is True:
            os.remove(new_dir + "/" + g + "_localisation_data.csv")
        if os.path.isfile(new_dir + "/" + g + "_no_local_output.csv") is True:
            os.remove(new_dir+"/" + g + "_no_local_output.csv")
        if os.path.isfile(new_dir+"/" + g + "_one_local_output.csv") is True:
            os.remove(new_dir + "/" + g + "_one_local_output.csv")
        if os.path.isfile(new_dir + "/" + g + "_two_local_output.csv") is True:
            os.remove(new_dir + "/" + g + "_two_local_output.csv")
        if os.path.isfile(new_dir + "/" + g + "_polar_output.csv") is True:
            os.remove(new_dir+"/" + g + "_polar_output.csv")
                    
        # set all files to a list, collect subset of text files (.txt)
        print(
            "Analysing files in: " + str(group[:-4]),
            "If there are any unrelated \".txt\" or \".csv\" files in this folder, this may cause errors. Open files may also cause errors.\n",
            sep="\n"
        )
        ...
        bad_cells = [] # cell names that could not be analysed
...
def sort_file(filename, group, file_ext):
    ...
    with open(filename, "r", encoding="ISO-8859-15") as ftxt:
        ...
        if not headers:     # couldn"t find ND or I in the file
            print(
                "File ", str(filename),
                " may not contain columns for ",
                "Normalized Distance and Intensity."
            )
...
def get_input_file(operating_system):
    ...
    print(
        "Formatted csv file should be in the current folder (directory).",
        "Make sure to include the \".csv\" file extension ",
        "when entering your filename.",
        sep="\n"
    )
    ...
    while True:
        ...
        if fname == "q":
            return "quit"
        elif os.path.isfile(sdir+"/"+fname) is True:
            return fname
...
def make_folder(ifile, current_OS):
    ...
    dirname = str(ifile[:-4])
    while True:
        if os.path.exists(sdir+"/"+dirname+str(counter)) is True:
            if counter == "":
                counter = 1
            else: 
                counter+=1
